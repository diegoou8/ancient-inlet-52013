...
const bogota = new Set(['bogota', 'bogotá', 'bogotá, d.c.', 'bogotá d.c', 'bogota dc.', 'bogota d.c', 'bogota dc','bogota ','bogotá d,c.','bogotá, d.c. ','bogotá d,c,','bogotá d,c. ' ].map(normalizeText));
...
app.post('/shipping', (request, response) => {
    ...
    try {
        ...
        if (totalItemPrice >= ORDER_TOTAL_THRESHOLD) {
            ...
            // If a reserva product is found, show the reserva shipping option based on the region
            ...
            // If no reserva product is found, show the regular shipping options based on the region
            else {
                if (bogota.has(normalizedCity)) {
                    shippingResults.push({
                        method: "Envío Bogotá",
